...
export default function Home() {
  ...
  return <main className="screen"><div className="bg"/><div className="grain"/><div className="wrap fadeUp"><div className="posterBox"><section className="card"><div className="kicker">你的影视原型</div><h1 className="resultTitle">{a.name}</h1><span className="pill">{a.title}</span><span className="pill">匹配度 {result!.match}%</span><p className="quote">{a.quote}</p>{a.tags.map(t=><span className="pill" key={t}>{t}</span>)}</section><section><canvas className="posterCanvas" ref={canvasRef}/><button className="btn full" style={{marginTop:12}} onClick={downloadPoster}>保存结果海报</button></section></div><section className="card section" style={{marginTop:18}}><h3>AI 长篇结果页</h3><p style={{whiteSpace:'pre-line'}}>{longText || '正在生成你的长篇结果……'}</p><h3>专属片单</h3><p>{a.films.join(' · ')}</p><div className="nav"><button className="btn" onClick={share}>复制邀请链接</button><button className="btn secondary" onClick={()=>{setPhase('start');setIdx(0);setAnswers(Array(questions.length).fill(-1));}}>重新测试</button></div></section></div></main>;
}
